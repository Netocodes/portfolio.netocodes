...
const NavbarComp = () => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 px-5 py-3 left-1/2 mt-5 -translate-x-1/2 z-[50] w-full md:w-11/12 flex flex-col  max-w-7xl items-center justify-between  bg-background/20 backdrop-blur-lg lg:rounded-full ${
          ...
          <div className="flex items-center gap-x-3">
            ...
            <Link href={"/"}>
              <Image
                src={LogoMain}
                alt="My main logo"
                className="w-40 bg-transparent rounded-lg  h-6.5 hidden dark:block "
                width={100}
                height={100}
              ></Image>
              <Image
                ...
                priority={true}
                alt="My main logo"
                className="w-40 bg-transparent rounded-lg  h-6.5 dark:hidden"
                width={100}
                height={100}
              ></Image>
            </Link>
          ...
          {navLinks && (
            <ul className="hidden  lg:flex underline underline-offset-4 decoration-[#1013d7] dark:decoration-cyan-300 gap-x-5 items-center justify-end">
              {navLinks.map((nav) => (
                <li
                  key={nav.id}
                  className="hover:bg-background/30 hover:py-3 hover:px-4 transition-all ease-in duration-300 rounded-lg"
                >
          ...
          <div className="flex items-center lg:flex-row-reverse gap-x-2 lg:gap-x-4">
            ...
            <div className="lg:hidden">
              <button
                ...
                className="cursor-pointer flex items-center justify-center rounded-full p-2 text-gray-700 hover:bg-gray-200 dark:text-gray-200 dark:hover:bg-cyan-300 dark:hover:text-gray-800 transition duration-300 ease-in-out"
              >
          ...
          <>
            <ul className="w-full flex lg:hidden flex-col items-center justify-center rounded-lg bg-background/20 backdrop-blur-lg my-4">
              {navLinks.map((nav) => (
                <Link
                  key={nav.id}
                  className="border-b shadow-lg w-full "
                  href={nav.link}
                >
