  ...
  const data: projectsProps[] = [
    {
      ...
      detatilDescription: "E-commerce platform built with React (Vite), TypeScript, and Node.js, enabling global skincare sales with manual payments and real-time currency conversion. Increased client’s sales by 45% while ensuring secure, scalable transactions.",
    ...
    },
    {
      ...
      detatilDescription: "  A healthcare portal built with Vite, Tailwind CSS, and Firebase (for secure login/registration) that connects patients to nearby hospitals, features medical blogs, and hosts charity events while managing guest access effectively. Increased engagement by 30%.",
      title: "Medical Plus",
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 px-4 gap-4">
        {data.map((item) => (
          ...
          >
            ...
            {/* Description */}
            <div className="px-3 ">
              <h2 className="text-2xl py-3 text-gray-800 dark:text-cyan-200 font-semibold mt-3">{item.title}</h2>
              <p className="text-md mt-4 text-gray-700 dark:text-gray-200">{item.detatilDescription}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 text-center px-3 py-4 gap-4">
                {item.techStack &&
                  item.techStack.map((tech, index) => (
                    ...
                    <span className="outline p-2 hover:outline-none hover:bg-blue-500/50 hover:dark:bg-cyan-300/50 outline-blue-500 dark:outline-cyan-300 rounded-full transition ease-in-out duration-300 " key={index}>
